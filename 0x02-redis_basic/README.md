...
+ [x] 1. **Reading from Redis and recovering original type**<br/>[exercise.py](exercise.py) contains a Python script with the following updates to the previous task:
  + Redis only allows storing strings, bytes, and numbers (and lists thereof). Whatever you store as single elements will be returned as a byte string. Hence, if you store `"a"` as a UTF-8 string, it will be returned as `b"a"` when retrieved from the server.
  ...
  + Remember to preserve the original `Redis.get` behavior if the key does not exist.
...
+ [x] 2. **Incrementing values**<br/>[exercise.py](exercise.py) contains a Python script with the following updates to the previous task:
  ...
  + Use the qualified name of `method` as a key using the `__qualname__` dunder method.
  ...
  + Remember that the first argument of the wrapped function will be `self`, which is the instance itself, allowing access to the Redis instance.
  + **Pro Tip**: When defining a decorator, it is useful to use `functools.wraps` to preserve the original function’s name, docstring, etc. Make sure you use it as described [here](https://docs.python.org/3.7/library/functools.html#functools.wraps).
...
+ [x] 3. **Storing lists**<br/>[exercise.py](exercise.py) contains a Python script with the following updates to the previous task:
  ...
  + In the new function that the decorator returns, use `rpush` to append the input arguments. Remember that Redis can only store strings, bytes, and numbers. Therefore, we can simply use `str(args)` to normalize. We can ignore potential `kwargs` for now.
